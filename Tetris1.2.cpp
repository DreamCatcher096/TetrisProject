	...
	for (int i = 0; i < 16; i++) {
		if ((piece.data[piece.orientation] & mask) != 0) {
			if (visible)
				...
				setfillcolor(BLACK);
			bar(piece.x + (PWIDTH * (i % 4)), piece.y + (i / 4 * PWIDTH), piece.x + PWIDTH + (PWIDTH * (i % 4)) - 1, piece.y + PWIDTH + (i / 4 * PWIDTH) - 1);
		}
		mask >>= 1;
	}
}
...
void draw_playfield(int *playfield)
{
	...
	for (int i = 0; i < 200; i++) {
		if (playfield[i]) {
			...
			bar(i % 10 * PWIDTH, i / 10 * PWIDTH, (i % 10 * PWIDTH + PWIDTH) - 1, (i / 10 * PWIDTH + PWIDTH) - 1);
		}
	}
...
void process_fallen_piece(tetris_piece_s *piece, int *playfield)
{
	...
	if (complete_lines > 0) {
		...
		_stprintf(name, _T("%d"), score); //方便查看每一个
...
int game_main()
{
	...
	// 绘制游戏区边界
	rectangle(-1, -1, WIDTH * UNIT + 1 - 1, HEIGHT * UNIT + 1 - 1);
	rectangle(-2, -2, WIDTH * UNIT + 2 - 1, HEIGHT * UNIT + 2 - 1);
	rectangle(-3, -3, WIDTH * UNIT + 3 - 1, HEIGHT * UNIT + 3 - 1);
	...
	score = 0;    //可测试i的值;i每次比信息少一个字符; 
	setcolor(RED);
	settextstyle(40, 0, _T("微软雅黑"));
	_stprintf(name, _T("%d"), score);
	outtextxy(-110, 290, name);
...
int GameOver(Person per[], char signName[], int v)
{
	...
	_stprintf(s, _T("%d"), per[0].score); //方便查看每一个
...
void RankScore(Person per[], char signName[], int v)
{
	...
	*/
	...
	for (l = 0; l<10; l++)
	{
		...
		ls[1] = l;
		_stprintf(s, _T("%d"), per[l].score);
		_stprintf(ls, _T("%d"), ls[1] + 1);
		settextstyle(30, 0, _T("微软雅黑"));
